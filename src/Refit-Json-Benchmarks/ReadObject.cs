...
{
    [MemoryDiagnoser]
    public class ReadObject : RefitBenchmark
    {
        ...
        public async Task<Organization> Read_Object_NewtonsoftJson()
            => await NewtonsoftJsonClient.GetOrganizationAsync("dotnet");

        [Benchmark]
        public async Task<Organization> Read_Object_SystemTextJson()
            => await SystemTextJsonClient.GetOrganizationAsync("dotnet");
    }
}
